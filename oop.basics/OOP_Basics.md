...
- С помощью метода equals который будет проверять идентичность. Либо compareTo. т.к. enum имплементирует интерфейс Comparable
...
- ссылки любого определенного типа, или к примеру подкласса `Trash< U extends TV>`
...
- исключения делятся на unchecked(ошибки которые могут произойти буквально в любом месте) и checked. В случае checked необходимо делать это вручную, по иерархии будут относиться следующие классы ReflectiveOperationExceptioni, SQLException, IOException.
...
- Данный оператор следует использовать при обработке checked исключений.
- один и больше.
